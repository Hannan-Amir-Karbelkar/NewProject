$(document).ready(function() {
	...
	/*$(document).on('click', '.IndicatorRightSide', function(){
		var pageId = 'collections' in pathMap ? pathMap['collections'] : 'index';
		alert(pageIdMap[pageId]);
	});*/
...
/*
    ...
    $(document).on('click', '.actionBtn', function(){
		...
		//var pageId = src in pathMap ? pathMap[src] : 'index';
        var id = $(this).data('id');
		...
		if(src == 'collections'){
			...
			if(mainid != ""){
				var name = $(this).closest('ul').find("li").first().data('name');
			}else{
		...
		history.pushState(null, '', url);*/
			...
			$('.categories-btn').filter(function(){
				return $(this).text().trim() === name;
			}).addClass('categories-active');
	...
	function loadContent(page, start, limit, com, proId, lazy, name, type, catType, mainid){
		var pageId = page in pathMap ? pathMap[page] : 'index'; //pathMap is an object that maps a URL's path to correspond hadler function
...
function bindLazyLoads(id, limit, start, action, text, type, catType, mainid){
	...
	//	var action = 'inactive';
			...
			start = start + 2;
				...
				alert(start + ' yahi se hota he bidu');
				action = 'active';
				start = start + 2;
				//alert(start);
